...
public class OracleTenantSessionTransactionRegressionTests
{
    [Fact]
    public void Dispose_WhenResetActionFails_SeversConnectionToPreventPoolContamination()
    {
        ...
        Func<DbConnection, Task> failingReset = _ => throw new TimeoutException("Simulated Oracle timeout during session reset!");

        var sessionTx = new OracleTenantSessionTransaction(innerTx, connection, failingReset);
    ...
    public void Dispose_MultipleCalls_DisposesInnerOnlyOnce()
    {
        ...
        var sessionTx = new OracleTenantSessionTransaction(innerTx, connection, _ => { resetCount++; return Task.CompletedTask; });
    ...
    public void Dispose_WhenResetActionSucceeds_InnerTransactionDisposedAndConnectionNotSevered()
    {
        ...
        var sessionTx = new OracleTenantSessionTransaction(innerTx, connection, successfulReset);
    ...
    public void Constructor_NullGuards_ThrowArgumentNullException()
    {
        ...
        var actNullTx = () => new OracleTenantSessionTransaction(null!, connection, reset);
        actNullTx.Should().Throw<ArgumentNullException>().WithParameterName("innerTransaction");

        var actNullConn = () => new OracleTenantSessionTransaction(innerTx, null!, reset);
        actNullConn.Should().Throw<ArgumentNullException>().WithParameterName("connection");

        var actNullReset = () => new OracleTenantSessionTransaction(innerTx, connection, null!);
    ...
    public async Task MethodsAndProperties_DelegateToInnerTransaction()
    {
        ...
        var sessionTx = new OracleTenantSessionTransaction(innerTx, connection, _ => Task.CompletedTask);
    ...
    public void Dispose_WithSyncResetAction_ExecutesSyncReset()
    {
        ...
        var sessionTx = new OracleTenantSessionTransaction(
            innerTx,
            connection,
    ...
    public async Task DisposeAsync_WhenConnectionClosed_DoesNotCallResetAction()
    {
        ...
        var sessionTx = new OracleTenantSessionTransaction(innerTx, connection, reset);
        await sessionTx.DisposeAsync();
    ...
    [Fact]
    public async Task SetTenantVpdContextAsync_WithOracleConnection_SetsClientIdDirectly()
    {
        using var connection = new global::Oracle.ManagedDataAccess.Client.OracleConnection();
        var tx = Substitute.For<DbTransaction>();
        var tenantId = TenantId.NewId();
        var context = Substitute.For<ITenantContext>();
        context.RequiredTenant.Returns(new TenantInfo(tenantId, "OracleTenant"));

        var act = () => connection.SetTenantVpdContextAsync(tx, context, setClientIdProperty: true);
    ...
    public async Task BeginTenantTransactionAsync_WhenRollbackThrows_PreservesOriginalException()
    {
        ...
        var context = Substitute.For<ITenantContext>();
        context.RequiredTenant.Returns(new TenantInfo(TenantId.Empty, "OracleTenant"));
