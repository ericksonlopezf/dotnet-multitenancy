...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier.
.DESCRIPTION
    Validates:
    ...
    4. Single distinct primary type per file in src/.
...
$docsFiles = Get-ChildItem -Path (Join-Path $RootDirectory "docs") -Recurse -Filter "*.md"
$badDocNames = 0
$docExceptions = @("README.md", "CHANGELOG.md", "CODE_OF_CONDUCT.md", "CONTRIBUTING.md", "SECURITY.md", "SUPPORT.md")
foreach ($doc in $docsFiles) {
    $filename = $doc.Name
    if ($docExceptions -contains $filename) {
        continue
    }
...
# 4. GitHub Repository Identity & Links
Write-Host "`n[4/7] Checking GitHub identity links (ericksonlopezf)..." -ForegroundColor Yellow
$badLinks = 0
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-multitenancy\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-multitenancy." -ForegroundColor Green }

# 5. Official Contact & Support Email Normalization
Write-Host "`n[5/7] Checking contact and security email normalization (ericksonlopezf@gmail.com)..." -ForegroundColor Yellow
$badEmails = 0
...
Write-Host "`n[6/7] Checking NoWarn suppressions in props and csproj files..." -ForegroundColor Yellow
$illegalNoWarn = 0
...
foreach ($proj in $projFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "<NoWarn>.*(CS0618|CS0619|CS1591|\b1591\b).*</NoWarn>") {
            Write-Host "  ❌ Prohibited warning suppression (CS0618/CS0619/CS1591) in $($proj.FullName):$($i + 1)" -ForegroundColor Red
            $violations++
            $illegalNoWarn++
        }
    }
}
if ($illegalNoWarn -eq 0) { Write-Host "  ✅ Zero prohibited NoWarn suppressions found (CS0618, CS0619, CS1591)." -ForegroundColor Green }

# 7. One Type Per File Verification
Write-Host "`n[7/7] Checking 'One Type Per File' rule in src/..." -ForegroundColor Yellow
$multiTypeViolations = 0
$srcFiles = Get-ChildItem -Path (Join-Path $RootDirectory "src") -Recurse -Filter "*.cs" | Where-Object { $_.FullName -notmatch "\\(obj|bin)\\" }
foreach ($cs in $srcFiles) {
    $code = Get-Content $cs.FullName -Raw
    $stripped = [regex]::Replace($code, '/\*.*?\*/', '', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    $stripped = [regex]::Replace($stripped, '//.*?$', '', [System.Text.RegularExpressions.RegexOptions]::Multiline)
    $matches = [regex]::Matches($stripped, '^(?:public|internal|file)\s+(?:sealed\s+|abstract\s+|static\s+|readonly\s+|partial\s+)*\b(class|struct|record(?:\s+struct)?|interface|enum)\s+([A-Za-z0-9_]+)(?:<[^>]+>)?', [System.Text.RegularExpressions.RegexOptions]::Multiline)
    $distinctNames = @($matches | ForEach-Object { $_.Groups[2].Value } | Select-Object -Unique)
    if ($distinctNames.Count -gt 1) {
        Write-Host "  ❌ Multiple distinct top-level types in $($cs.FullName): $($distinctNames -join ', ')" -ForegroundColor Red
        $violations++
        $multiTypeViolations++
    }
}
if ($multiTypeViolations -eq 0) { Write-Host "  ✅ All source files adhere to 'One Type Per File' convention." -ForegroundColor Green }

# -----------------------------------------------------------------------------
# Implicit Usings Consistency Gate
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Implicit Usings] Validating consistent Implicit Usings policy..." -ForegroundColor Yellow
$implicitErrors = 0
$bldProps = Join-Path $RootDirectory "Directory.Build.props"
if (Test-Path $bldProps) {
    $bldContent = Get-Content $bldProps -Raw
    if ($bldContent -notmatch '<ImplicitUsings>\s*enable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Directory.Build.props does not enforce <ImplicitUsings>enable</ImplicitUsings>." -ForegroundColor Red
        $violations++
        $implicitErrors++
    }
}
$allProjs = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.csproj" | Where-Object { $_.FullName -notmatch "\\(obj|bin)\\" }
foreach ($p in $allProjs) {
    $pContent = Get-Content $p.FullName -Raw
    if ($pContent -match '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
        Write-Host "  ❌ Disabled ImplicitUsings in $($p.FullName)." -ForegroundColor Red
        $violations++
        $implicitErrors++
    }
}
if ($implicitErrors -eq 0) { Write-Host "  ✅ Implicit Usings consistently enabled across all projects." -ForegroundColor Green }

# -----------------------------------------------------------------------------
# Version Harmony Gate (Directory.Build.props & Telemetry)
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Version Harmony] Validating authoritative version synchronization..." -ForegroundColor Yellow
$versionErrors = 0
$authoritativeVersion = "2.0.0"
if (Test-Path $bldProps) {
    $bldContent = Get-Content $bldProps -Raw
    if ($bldContent -match '<VersionPrefix>\s*([^<\s]+)\s*</VersionPrefix>') {
        $authoritativeVersion = $Matches[1]
    }
}
$telemetryFile = Join-Path $RootDirectory "src/EricksonLopez.MultiTenancy.OpenTelemetry/TenantActivitySource.cs"
if (Test-Path $telemetryFile) {
    $telContent = Get-Content $telemetryFile -Raw
    if ($telContent -notmatch [regex]::Escape("Source = new(ActivitySourceName, `"$authoritativeVersion`")")) {
        Write-Host "  ❌ TenantActivitySource version does not match authoritative version ($authoritativeVersion)." -ForegroundColor Red
        $violations++
        $versionErrors++
    }
}
$metricsFile = Join-Path $RootDirectory "src/EricksonLopez.MultiTenancy.OpenTelemetry/TenantMetrics.cs"
if (Test-Path $metricsFile) {
    $metContent = Get-Content $metricsFile -Raw
    if ($metContent -notmatch [regex]::Escape("MeterVersion = `"$authoritativeVersion`"")) {
        Write-Host "  ❌ TenantMetrics version does not match authoritative version ($authoritativeVersion)." -ForegroundColor Red
        $violations++
        $versionErrors++
    }
}
if ($versionErrors -eq 0) { Write-Host "  ✅ Authoritative version ($authoritativeVersion) synchronized across telemetry." -ForegroundColor Green }
