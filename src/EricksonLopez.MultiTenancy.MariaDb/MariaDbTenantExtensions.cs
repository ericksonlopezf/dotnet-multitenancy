...
/// <summary>
/// Provides MariaDB session variable and transaction-scoped context integration for multi-tenant data access.
/// Sets a connection/session user variable (e.g. <c>@app_tenant_id</c>) to allow multi-tenant views, stored routines,
/// triggers, and queries to enforce tenant boundaries in MariaDB.
/// </summary>
/// <remarks>
/// <para>
/// Security model: MariaDB supports session user variables with <c>@</c>.
...
public static class MariaDbTenantExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new TenantNotFoundException("Tenant identifier is empty. Cannot establish MariaDB tenant session variable.");
        }
    ...
    /// <summary>
    /// Resets the MariaDB tenant user variable to <see langword="null"/> to prevent context leakage across connection pool reuse.
    /// </summary>
    /// <param name="connection">The open database connection.</param>
    /// <param name="transaction">The optional active MariaDB transaction.</param>
    /// <param name="variableName">The user variable name (without the '@' prefix). Defaults to <see cref="DefaultTenantVariableName"/>.</param>
