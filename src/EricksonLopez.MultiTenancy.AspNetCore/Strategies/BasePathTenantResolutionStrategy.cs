...
public sealed class BasePathTenantResolutionStrategy : ITenantResolutionStrategy
{
    ...
    private static bool TryGetPathSegment(string path, int targetIndex, out string segment)
    {
        ...
        while (!span.IsEmpty)
        {
            ...
            // Stryker disable once statement, block : Loop termination
            if (span.IsEmpty)
            {
                break;
            }
        ...
        // Stryker disable string, boolean : Fallback empty string and return false
        segment = string.Empty;
        return false;
        // Stryker restore string, boolean
    }
}
