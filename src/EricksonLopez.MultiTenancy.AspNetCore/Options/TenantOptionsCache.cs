    ...
    where TTenant : class, ITenantInfo
{
    ...
    /// <exception cref="ArgumentNullException"><paramref name="httpContextAccessor"/> is <see langword="null"/></exception>
    // Stryker disable null coalescing, boolean : Options configuration defaults
    ...
    // Stryker restore null coalescing, boolean
    ...
    private void RecordAccess(string key)
    {
        if (_lruIndex.TryGetValue(key, out var node))
        {
            if (node.Previous is not null && Monitor.TryEnter(_lruLock))
            {
                ...
                {
                    // Stryker disable once statement : Lock release
    ...
    public void Clear()
    {
        lock (_lruLock)
        {
            // Stryker disable once statement : LRU list clearing
            _lruList.Clear();
            // Stryker disable once statement : LRU index clearing
            _lruIndex.Clear();
        }
        _innerCache.Clear();
    }
    ...
    public bool TryAdd(string? name, TOptions options)
    {
        ...
        // Stryker disable once boolean, statement : LRU access tracking on addition
        if (added)
        {
            RecordAccess(key);
        }
    ...
    public bool TryRemove(string? name)
    {
        ...
        // Stryker disable boolean, statement, equality : LRU eviction cleanup on removal
        if (removed)
        {
            lock (_lruLock)
            {
        ...
        // Stryker restore boolean, statement, equality
        return removed;
    }
}
