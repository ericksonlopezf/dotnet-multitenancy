...
/// <summary>
/// Represents a <see cref="DbTransaction"/> decorator that automatically resets SQL Server session context upon transaction disposal,
/// ensuring that pooled connections returned to ADO.NET do not retain tenant context.
/// </summary>
internal sealed class TenantSessionTransaction : DbTransaction
{
    ...
    public TenantSessionTransaction(
    ...
    protected override void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            if (disposing)
            {
                ...
                catch
                {
                    ...
                    // If session reset fails (e.g. transient timeout or network glitch), sever and close
                    // the physical connection immediately so ADO.NET discards the socket rather than
                    // returning a tainted connection to the pool.
                    SeverConnection();
                }
                finally
                {
        ...
        // Stryker disable once statement : Base DbTransaction disposal
    ...
    private void SeverConnection()
    {
        ...
        {
            _connection.Close();
            // Stryker disable once statement : Defensive disposal of severed connection
            _connection.Dispose();
        }
        catch
        {
