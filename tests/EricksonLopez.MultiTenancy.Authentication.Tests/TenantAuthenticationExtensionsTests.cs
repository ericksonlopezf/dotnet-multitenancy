...
public class TenantAuthenticationExtensionsTests
{
    ...
    [Fact]
    public void AddPerTenantAuthentication_WithOptions_NullServices_ThrowsArgumentNullException()
    {
        IServiceCollection services = null!;
        var act = () => services.AddPerTenantAuthentication<TenantInfo>(_ => { });
        act.Should().Throw<ArgumentNullException>().WithParameterName("services");
    }
    ...
    public void AddPerTenantAuthentication_WithOptions_RegistersServicesAndConfiguresOptions()
    {
        ...
        var opt = sp.GetRequiredService<IOptions<TenantAuthenticationOptions>>().Value;
        opt.DefaultSchemeSelector.Should().NotBeNull();
    }
