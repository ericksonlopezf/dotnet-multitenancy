...
public sealed class SuperuserConnectionStringAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeLiteral(SyntaxNodeAnalysisContext context)
    {
        ...
        // Stryker disable once logical, equality, statement, block : Minimum connection string length heuristic
        if (string.IsNullOrWhiteSpace(text) || text.Length < 10)
        {
            return;
        }

        // Stryker disable once statement, block : Guard clause for connection string
